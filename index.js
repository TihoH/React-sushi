...
const editDataUser = require("./modules/editDataUser");
const adedFavories = require("./modules/adedFavories");
...
app.get("/search", async (req, res) => {
  ...
  try {
    if (!q || q.trim() === "") {
      return res
        .status(400)
        .json({ message: "Параметр запроса 'q' обязателен" });
    }
...
app.use("/", loginUser);
app.use("/", getUser);
app.use("/", editDataUser);
app.use("/", adedFavories);
